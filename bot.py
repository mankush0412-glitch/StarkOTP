...
async def _session_file_to_string(session_bytes: bytes) -> Optional[str]:
    ...
    Hard timeout of 25 s per session so one bad file can't freeze the whole ZIP.
    """
    ...
    async def _do() -> Optional[str]:
        ...
        # NOTE: We do NOT call is_user_authorized() here because sessions created
        # with a different API_ID will always fail that check even if the auth_key
        # is perfectly valid. The live AccountManager client will reject it if it's
        # truly invalid when it calls start().
        ss = StringSession.save(client.session)
        log.info(f"[zip] ✅ Converted {fmt} session → StringSession (auth check skipped)")
        return ss

    try:
        # Hard 25-second timeout — prevents any single session from hanging the bot
        return await asyncio.wait_for(_do(), timeout=25)

    except asyncio.TimeoutError:
        log.warning(f"[zip→str] {fmt} session timed out after 25 s — skipping")
        return None
    except Exception as e:
...
async def process_session_zip(
    ...
    total      = len(all_names)
    sem        = asyncio.Semaphore(3)   # max 3 concurrent Telegram connections
    ...
    async def _update_progress():
        ...
        with contextlib.suppress(Exception):
            await prog_msg.edit(
                ...
                "_Verifying with Telegram (3 at a time)…_"
            )
...
async def message_handler(event):
    ...
    # ── ADMIN: ZIP file ───────────────────────────────────────
        ...
        await prog.edit(
            ...
            f"🔄 Verifying with Telegram (3 at a time)…\n\n"
