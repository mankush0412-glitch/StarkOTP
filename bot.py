...
def _session_file_to_string(session_bytes: bytes) -> Optional[str]:
    """
    ...
    This means sessions created with ANY API_ID (including third-party apps) are
    accepted; the live AccountManager client will tell us if the session is truly
    invalid when it first connects.
    """
    ...
    if ss:
        log.info(f"[zip] ✅ {fmt} session → StringSession (offline, no API_ID check)")
    else:
