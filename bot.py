...
async def _session_file_to_string(session_bytes: bytes) -> Optional[str]:
    ...
    - Telethon format : opened directly via TelegramClient
    - Pyrogram format : auth_key + dc_id extracted, StringSession built manually,
                        then verified via TelegramClient

    Returns None if the session is not authorized or conversion fails.
    """
    ...
    try:
        if fmt == "telethon":
            # ── Telethon: write file, open normally ───────────
            ...
            # ── Pyrogram: build StringSession from raw bytes ──
            ss_str = _pyrogram_to_telethon_ss(dc_id, server, port, auth_key)
            if not ss_str:
                return None
            client = TelegramClient(
                StringSession(ss_str), API_ID, API_HASH,
                connection_retries=1, timeout=15,
            )
        ...
        if not await client.is_user_authorized():
            log.warning(f"[zip] {fmt} session not authorized — skipping")
            return None

        ss = StringSession.save(client.session)
        log.info(f"[zip] ✅ Converted {fmt} session → StringSession")
        return ss

    except Exception as e:
        log.warning(f"[zip→str] {fmt} conversion failed: {e}")
        return None
    finally:
        if client:
            with contextlib.suppress(Exception):
...
async def process_session_zip(
    ...
    sem = asyncio.Semaphore(5)   # max 5 concurrent Telegram connections
    ...
    async def _process_one(name: str):
        ...
        async with sem:
            ...
            try:
                ...
                log.warning(f"[zip] Could not start live client for {phone}: {e}")

            added += 1
            log.info(f"[zip] ✅ Added {phone} ({country_name})")

    await asyncio.gather(*[_process_one(n) for n in zf.namelist()])
    zf.close()
...
# ══════════════════════════════════════════════════════════════
def _make_upi_qr(upi_id: str, amount: float, name: str) -> bytes:
...
async def message_handler(event):
    ...
    if state == "deposit_amount":
        ...
        step2_note = (
            "\n\n**Step 2 — After paying:**\n"
            "📸 Send the **payment screenshot** (photo)\n"
            "You can also add UTR/ref number in the photo caption (optional)"
        )
        try:
    ...
    # ── DEPOSIT: user sends payment screenshot (photo) ───────
    elif state == "deposit_screenshot":
        amount = state_data["amount"]

        # Must be a photo — if user sent text, remind them
    ...
    # ── ADMIN: ZIP file ───────────────────────────────────────
        ...
        await prog.edit(
            ...
            "Verifying each .session file with Telegram.\n"
            "This may take a moment depending on how many files are in the ZIP."
        )
        ...
        try:
            added, skipped, errors = await process_session_zip(
                zip_bytes, cc, cname, price, twofa
            )
